...
function load_more_goods($page){
	$("div[id='load']").show(10);
	$.ajax({
		url:APP+"/goods/ajaxlookfor",
		type :'post',
		dataType:'json',
		async : false,
		data :{'page':$page,'keyword':$("#keyword").attr('data-value'),'_token':$('meta[name="_token"]').attr('content')},
			...
			if(data.status == 'success'){
				...
				if(list.length != 0 && !in_array($page,needle)) {
					...
					for(var one in list) {
	                    ...
	                    $boxes += '<p><a href="'+APP+'/goods/detail/'+list[one]['id']+'" class="goods_block_a"><button class="btn btn-primary" onclick="window.location.href="'+PUBLIC+''+list[one]['img_thumb_path']+'">查看详情</button></a>';
	                    $boxes += '</p></div></div></div>';
					}
					var el = jQuery($boxes);
