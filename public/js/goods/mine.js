	...
	$.ajax({
		...
		success:function(data){
			if(data.status == 'success'){
				...
				if(list.length != 0) {
					for(var one in list) {
						var $boxes = '<a href="'+APP+'/goods/detail/'+list[one]['id']+'" class="goods_block_a"><div class="col-md-3 col-xs-12 item"><div class="thumbnail" id="goods_block">';
	                    ...
	                    $boxes += '<p><a href="'+APP+'/goods/detail/'+list[one]['id']+'" class="btn btn-primary" role="button">查看详情</a> </p></div></div></div></a>';
						var el = jQuery($boxes);
